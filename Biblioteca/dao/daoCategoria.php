    ...
    public function salvar($source)
    {
        try {
            if ($source->getIdCategoria() != "") {
                $statement = Conexao::getInstance()->prepare("UPDATE tb_categoria SET nomeAutor=:nome WHERE idtb_categoria = :id;");
                $statement->bindValue(":id", $source->getIdCategoria());
            } else {
    ...
    public function atualizar($source)
    {
        try {
            ...
            if ($statement->execute()) {
                ...
                $source->setIdtbAutores($rs->idtb_categoria);
    ...
    public function tabelapaginada()
    {
     ...
     <tbody>";
            foreach ($dados as $source):
                echo "<tr>
        <td style='text-align: center'>$source->idtb_categoria</td>
        <td style='text-align: center'>$source->`nomeCategoria`</td>
        <td style='text-align: center'><a href='?act=upd&id=$source->idtb_categoria' title='Alterar'><i class='ti-reload'></i></a></td>
        <td style='text-align: center'><a href='?act=del&id=$source->idtb_categoria' title='Remover'><i class='ti-close'></i></a></td>
       </tr>";
            endforeach;
            echo "
</tbody>
